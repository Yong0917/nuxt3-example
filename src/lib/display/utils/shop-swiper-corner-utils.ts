import { IsEmpty, IsEmptyArrayCheck } from '@/utils/common'
import type {SetInfo} from "~/types/shop-data-model";
import type {SwiperModel} from "~/types/swiper-data";
 ...
 * @return SwiperModel[]
 * */
export const getSwiperCornerContent = ({ setList = [], maxCnt }: SwiperCornerProps) => {
  const contentList = setList.slice(0, maxCnt ?? setList.length)
    .filter(({ swipeTyp, contentInfoList }) => swipeTyp && (contentInfoList ?? []).length >= SWIPER_MIN_CONTENTS )
...
export const getSwiperModelBySetInfo = (setInfo: SetInfo) => {
  ...
  setInfo.contentInfoList?.forEach(({ imageList, textList } : any) => {
