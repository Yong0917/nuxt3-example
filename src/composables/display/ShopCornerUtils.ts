...
import type {Brand, Content, ContentInfo, GoodsReview, SetInfo} from "~/types/shop-data-model";
import type {GoodsSummary, PlanInfoList} from "~/types/planType";
...
export const getCornerContent = ({
    ...
    const contentList = setList?.filter(setCondition)?.map((setInfo : any) => {
        const data: any = {}
        setInfo.contentInfoList?.forEach((contentInfo: any) => {
            const target : any = CornerTarget[contentInfo.conrTgtCd]
            const prop: any = props[target.propKey] // 코너대상코드 해당 prop
            ...
            if (prop) {
                ...
                // 최소 노출 개수 검증
                if (size(contents) >= (prop.minCnt ?? 1)) {
